...
public class SegmentRouting {
    ...
    public static Node[] getSegments(Graph graph, Path assigned_path) throws Exception {
        List<Node> ap_nodes = assigned_path.getNodePath();  //获取分配路径上的节点
        Node dst = ap_nodes.get(ap_nodes.size() - 1);   //获取destination节点
        ArrayList<Node> segments = new ArrayList<>();   //segments数组
        Node segment;
        while (!(segment = getFirstSegment(graph, assigned_path)).equals(dst)) {
            //如果得到的第一个Segment节点不等于destination节点
            //则反复迭代
            segments.add(segment);  //则segments添加该节点
            debug("getSegments(): segment: " + segment);
            assigned_path = subpath(assigned_path, segment);    //截取子路径
        }
        // add destination as last segment
        segments.add(dst);  //segments添加destination
    ...
    static Node getFirstSegment(Graph graph, Path assigned_path) throws Exception {
        debug("getFirstSegment(): assigned_path: " + assigned_path);
        List<Node> ap_nodes = assigned_path.getNodePath();  //获取路径上的节点
        Node src = ap_nodes.get(0); //获取source节点
        Node dst = ap_nodes.get(ap_nodes.size() - 1);   //获取destination节点
        Iterator<Path> natural_paths = getNaturalPaths(graph, src.getId(), dst.getId()).iterator(); //获取从source到destination的所有naturalPaths

        if (!natural_paths.hasNext()) throw new Exception("No natural path has been found");    //如果没有则抛异常
        // else, natural path exists
        Path first_natural_path = natural_paths.next();     //获取第一条naturalPath
        if (!natural_paths.hasNext()) {
            // only one natural path
            if (same(first_natural_path, assigned_path)) {  //如果唯一一条naturalPath和assignedPath相等
                debug("getFirstSegment(): found");
                return dst; //则返回destination节点
            }
            // else continue
        }
        // else, the two path differ
        Path sub_path = assigned_path.getACopy();   //否则复制assignedPath
        sub_path.popNode(); //弹出最后一个节点
        return getFirstSegment(graph, sub_path);    //将弹出节点后的subPath带入继续求解
    }
     ...
     */
    public static Path getNaturalPath(Graph graph, String src, String dst) {
        Iterator<Path> i = getNaturalPaths(graph, src, dst).iterator(); //获取从src到dst的naturalPaths
        if (i.hasNext()) return i.next();   //返回一个从source到destination的等效路径
        else return null;
    }
    ...
    public static Iterable<Path> getNaturalPaths(Graph graph, String src, String dst) {
        debug("getNaturalPaths(): src=" + src + " dst=" + dst);
        Node src_node = graph.getNode(src); //获取source节点
        Node dst_node = graph.getNode(dst); //获取destination节点
        //构建Dijkstra对象
        //以跳作为长度
        Dijkstra dijkstra = new Dijkstra();
        dijkstra.init(graph);
        dijkstra.setSource(src_node);   //设置source节点
        dijkstra.compute();     //生成最短路树
        Iterable<Path> paths = dijkstra.getAllPaths(dst_node);      //获取所有到destination的路径
        int count = 0;
        for (Path p : paths) debug("getNaturalPaths(): ECSP[" + (count++) + "]: " + p);
        return paths;   //返回paths
    }
